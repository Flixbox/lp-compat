...
You can leave those checkboxes as-is, but applying these patches would be good.
...
- Applying the _Removes integrity check and signature verification_ and _Fake a modified APK archive from the original_ patches alongside the other patches.
- Instead of _Support patch for InApp emulation (Reassembly Dex)_, try both _Support patch for LVL and Inapp emulation_ and _Support patch for Inapp emulation (Proxy server)_.

Finally, install the modified APK file by tapping the "Go to file" button and then the "Uninstall and install" button. Afterward, press "Yes", then "Ok", then finally "Install".

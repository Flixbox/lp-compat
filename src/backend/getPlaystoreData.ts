...
const SCRAPER_BASE = "https://lp-compat.vercel.app";
...
export default async function getPlaystoreData(appId: string): Promise<PlayStoreData | null> {
  if (!appId) return null;

  async function postTo(method: string, body: unknown) {
    const res = await fetch(`${SCRAPER_BASE}/${method}`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Scraper ${method} responded with ${res.status}: ${text}`);
    }
    return res.json();
  }
  ...
  try {
    ...
    try {
      ...
      try {
        const searchRes = (await postTo("search", { term: appId, num: 1 })) as any[];
        found = Array.isArray(searchRes) && searchRes.length > 0 ? (searchRes[0] as PlayStoreData) : null;
      } catch (err) {
    ...
    if (!found.installs && (found.minInstalls !== undefined)) found.installs = found.minInstalls;
    if (!found.scoreText && typeof found.score === "number") found.scoreText = String(found.score.toFixed(1));
